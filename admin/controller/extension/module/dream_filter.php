...
class ControllerExtensionModuleDreamFilter extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'] = array(
			...
			array(
				...
				'href' => $this->url->link('extension/extension', $token_url . '&type=module', true)
			)
		);
        ...
        if(version_compare(phpversion(), '5.6', '<')) {
            ...
            $data['errors'][] = sprintf($language['error_modification'], $this->url->link('extension/modification', $token_url, true));
        } else {
            ...
            $data['activate'] = $this->url->link($this->_route . '/activate', $token_url, true);
        }

        $data['cancel'] = $this->url->link('extension/extension', $token_url . '&type=module', true);
		$data['updates_btn'] = $this->url->link($this->_route . '/updates', $token_url, true);
	...
	public function edit() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate($this->request->post, $languages)) {
			...
			$this->response->redirect($this->url->link('extension/extension', $token_url . '&type=module', true));
		}
		...
		$data['action'] = $this->url->link($this->_route . '/edit', $token_url . ($module_id ? '&module_id=' . $module_id : ''), true);
		$data['home'] = $this->url->link($this->_route, $token_url . ($module_id ? '&module_id=' . $module_id : ''), true);
		$data['apply'] = $module_id ? $this->url->link($this->_route . '/apply', 'module_id=' . $module_id . '&' . $token_url, true) : false;
        $data['version'] = $this->version;
		...
		$data['cancel'] = $this->url->link('extension/extension', $token_url . '&type=module', true);
		$data['activate'] = $this->url->link($this->_route . '/activate', $token_url, true);
		$data['cache_btn'] = $this->url->link($this->_route . '/cache', $token_url, true);
		$data['reset_btn'] = $this->url->link($this->_route . '/reset', $token_url, true);
		$data['updates_btn'] = $this->url->link($this->_route . '/updates', $token_url, true);
		$data['token'] = $token;
		...
		$form_new = array(
			'id' => 'new',
			'action' => $this->url->link($this->_route . '/filter', $token_url . ($module_id ? '&module_id=' . $module_id : ''), true),
			'token' => $token,
		...
		if(!empty($data['filters']) && is_array($data['filters'])) {
		    ...
		    foreach ($data['filters'] as $id => $filter) {
				...
				$filter_forms[] = $this->load->view($this->_route . '_form', array(
					'id' => $id,
					'action' => $this->url->link($this->_route . '/filter', $token_url . ($module_id ? '&module_id=' . $module_id : '') . '&id=' . $id, true),
					'token' => $token,
	...
	public function filter() {
		...
		if(!isset($this->request->get['id']) && !isset($json['errors'])) {
			...
			$json['modal'] = $this->load->view($this->_route . '_form', array(
				'id' => $json['id'],
				'action' => $this->url->link($this->_route . '/filter', $token_url . ($module_id ? '&module_id=' . $module_id : '') . '&id=' . $json['id'], true),
				'token' => $token,
