...
class ControllerCommonHeader extends Controller {
 ...
 if (defined('DIR_CATALOG')) { $data['show_lightning'] = $this->user->hasPermission('modify', 'setting/setting'); if( $data['no_lightning'] = !is_file(DIR_CATALOG.'controller/extension/lightning/gamma.php')) $data['show_lightning'] = 0; if ($data['show_lightning']) if (($k = glob(DIR_CACHE.'lightning/core/*.key')) && filemtime($k[0]) > time() - 24*60*60) $data['show_lightning'] = substr($k[0], strrpos($k[0], '/')+1, -4); else { if ($k) @unlink($k[0]); @file_put_contents(DIR_CACHE . 'lightning/core/' . ($data['show_lightning'] = sha1(rand() . rand() . rand() . rand())) . '.key', '');} if (!empty($this->request->get['j_edit'])) $data['show_lightning'] = 0; } // Lightning 
