...
final class DB {
 ...
 if (function_exists('Wnh')) Wnh($session_id, $query); // Lightning 
		if ($query->num_rows) {
 ...
 if (function_exists('Wni')) { if (!empty($this->expire)) if (Wni($session_id, $data, $this->expire)) return true; if (!empty($this->maxlifetime)) if (Wni($session_id, $data, $this->maxlifetime)) return true; } // Lightning 
