...
$this->url->addRewrite(new Simple\Rewrite($this->config, $this->session));
 ...
 if (function_exists('Wng') && Wng($this->request->cookie)) $code = $this->request->cookie['language']; // Lightning 
		if (isset($this->request->cookie['language']) && !array_key_exists($code, $languages)) {
			$code = $this->request->cookie['language'];
		}
		
		// Language Detection
		 if (function_exists('Wmn') && Wmn()) if (!empty($this->request->server['HTTP_ACCEPT_LANGUAGE'])  && !array_key_exists($code, $languages)) {
			$detect = '';
